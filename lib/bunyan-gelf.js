...
GelfStream.prototype.write = function (record, callback) {
    ...
    var type = Object.prototype.toString.call(record);
    try {
    ...
    var that = this;
    return this.gelf.emit('gelf.log', this.createMessage(record), function (err) {
        that.pendingMessageCount--;
        if (callback) {
            callback(err);
        }
        if (that.pendingMessageCount == 0) {
            that.emit("drain");
        }
    });
};
